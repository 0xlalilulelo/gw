...
//!
//! Mirrors the public contract of [`gw_codegen_fast::compile_program`]:
//! `MirProgram → object bytes`. The driver picks between the two backends
//! at `gw build --backend=fast|llvm`.
//!
...
pub fn compile_program(
    ...
    // `gw_codegen_fast`. Release-mode tuning is a Phase 8+ concern.
    let machine = target
    ...
    // global's address. Mirrors `gw_codegen_fast`'s rodata pass —
...
/// Mirrors `gw_codegen_fast::is_aggregate_ty` — same rule on both
