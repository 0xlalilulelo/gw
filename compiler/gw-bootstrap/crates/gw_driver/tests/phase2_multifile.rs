...
//! whole subdirectory into a per-test temp dir, runs `gw build
...
fn unique_tmp(name: &str) -> PathBuf {
    ...
    p.push(format!("gw-phase2-multifile-{name}-{pid}-{nanos}"));
...
fn run_corpus(backend: &str) {
    ...
    for project in projects {
        ...
        let build = Command::new(&gw)
            .args(&build_args)
            .output()
            .expect("spawn gw build");
        if !build.status.success() {
            panic!(
                "gw build failed for {project_name}\nstdout: {}\nstderr: {}",
