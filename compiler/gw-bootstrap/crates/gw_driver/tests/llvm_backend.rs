...
//! `gw build --backend=llvm`, the resulting executable is run, and
...
fn unique_tmp(name: &str) -> PathBuf {
    ...
    p.push(format!("gw-llvm-{name}-{pid}-{nanos}"));
...
fn llvm_backend_runs_full_corpus() {
    ...
    for entry in entries {
        ...
        let build = Command::new(&gw)
            .args(&build_args)
            .status()
            .expect("invoke gw build --backend=llvm");
        assert!(
            build.success(),
            "`gw build --backend=llvm {}` failed (status {build:?})",
            staged.display()
        );
