...
//! is built via `gw build`, the resulting executable is run, and
...
fn gw_binary() -> PathBuf {
    ...
    // binary lives one level up at `<profile>/gw`. Use the
    // OUT_DIR-relative trick that cargo's `env!("CARGO_BIN_EXE_gw")`
    // affords us — the macro returns the absolute path to the
    // gw binary built for tests.
...
fn unique_tmp(name: &str) -> PathBuf {
    ...
    p.push(format!("gw-phase1-{name}-{pid}-{nanos}"));
