//! Integration tests for the `gw` driver. Exercise `--version`,
//! `--help`, `gw new`, and `gw build` through the in-process
//! `gw_driver::run` entry point so the same code path the binary
...
fn unique_tmp(name: &str) -> PathBuf {
    ...
    p.push(format!("gw-test-{name}-{pid}-{nanos}"));
