...
the 258-program Phase-1 corpus + 4 multi-file projects + the 31
...
### What 289 corpus programs cover
...
bit-exactly the same value across all 289 single-file corpus
programs (258 phase1 + 31 phase2_comptime) + 4 multi-file
projects.
...
B.3 = 1 caught (`lower_match` dangling-block terminator was
`Goto(join_bb)` rather than `Unreachable`, poisoning the
must-init join), CT.1's E0315
...
prediction stays armed for CT.3c. B.0's prediction was ~1-2
(new MIR rvalue family + new typeck Ty variant + the
"address-taken locals go to stack slots" storage decision on
the Cranelift side); observed yield was 0 because Rust's
exhaustive-match checking surfaced every missing arm at compile
time, the LLVM codegen's "every local gets an alloca" shape
made `Rvalue::Ref` literally `cx.allocas[target]`, and the
Cranelift `address_taken_locals` metadata is small enough to
inspect by hand. The recombination rule predicted B.1 (`&mut`)
rides on B.0 cleanly because the only addition is a mutability
bit at the type level; storage / codegen shape stays identical.
B.1's observed yield was 1, half the prediction of "rides
cleanly with zero new bugs": the variant migration *did* land
zero-bug end-to-end (every consumer pattern auto-updated under
exhaustive-match), but `synth_assign`'s deref-LHS arm — added
to type-check the new `*r = v` shape — computed the LHS type
locally without recording it into `expr_types`, so MIR's
`StoreThroughRef` saw `Ty::Error` for the store width whenever
the RHS was a Local. The const-RHS smoke test masked the bug
because LLVM's `read_operand` const path ignores the `ty`
argument. The lesson for B.2+ is that any new typeck arm that
synthesises an LHS / pattern type without going through
...
B.3 observed yield was 1, the lower end of the predicted
"1-2" — and notably the bug was a **MIR-level fix**
(`lower_match` dangling-block terminator), not a
checker-implementation bug. The dataflow / worklist / lattice
machinery landed clean on first try, which is what we hoped
for from "incremental closure" — the framework itself is
small and self-contained, the riskier part is wiring it into
the existing MIR semantics where load-bearing assumptions
(like "all blocks reach `join_bb` via a value-writing path")
turn out to be implicit. The dataflow's must-init
intersection surfaced the latent `Goto(join_bb)` issue
precisely because it asked the question "is the match-result
local definitely written on *every* incoming edge to the
join?" — a question the existing codegen never asked because
both backends happened to read garbage from the same
underlying alloca on the never-taken path. **The pattern
across B.1 / B.2 / B.3: the *checker* increment is also a
*validator* increment for the foundational increments it
sits on top of.** This is exactly the design rationale for
the Phase-3 ladder ordering — by the time the borrow checker
proper (B.5) lands, B.3's framework will have flushed the
"latent MIR-level garbage-read" shapes, and B.4's region
inference will have flushed the "latent reference-shape"
shapes. Whatever bugs B.5 reveals will be checker-design
bugs, not foundational ones.

B.4 observed yield was 0 against a prediction of ~1. The
checker-increment-as-validator pattern from B.1 / B.2 / B.3
*did* run — `check_region_origins` exercises every existing
`Rvalue::Ref` site, every `Terminator::Call` returning a ref,
and every `Return(Local(r))` with a ref-typed `r` — but
found nothing latent because B.0–B.3 had already validated
those shapes through the codegen path. The fixpoint
machinery itself landed clean on first try: the lattice's
monotone-only-grows shape made the convergence argument
structural; the `is_ref(f, l)` predicate routed through
`Ty::Ref { .. }`'s struct-variant shape already validated
by B.1's migration; and the parameter / let-scope
discriminator went through the same `f.params` set the rest
of the borrow checker already consults. **The recombination
rule's prediction "checker increments are also validators of
foundational increments" still held — it's just that the
foundational work was exhaustive enough by B.3 that B.4's
validator role found a clean tree.** This is the rule's
*success case* surfacing visibly: when the checker-increment
yield drops to 0, it's evidence that the cumulative
foundational work has covered the surface, not that the
checker increment was trivial. The same predictive
implication carries forward: B.5's loan-tracking lattice will
exercise B.0–B.4's reference / origin shapes again under a
flow-sensitive lens, and whatever it finds will be
checker-design bugs (false positives on aliasing, missed
...
Part D.5 "Polonius lite" design is staged as a nine-step ladder
mirroring CT.x's incremental closure:

- **B.0 borrow-surface tracer is closed** (commit `f85a94e`):
  `&local` and `*r` parse, type-check, and lower end-to-end as
  the canonical pointer-to-storage shape. `Ty::Ref(RefInner)`
  with `RefInner = Int(IntTy) | Bool` (closed inner like
  `OptInner`, keeps `Ty: Copy`). `Rvalue::Ref { target,
  pointee_ty }` and `Rvalue::Deref { ptr, ty }` in MIR;
  `MirFn::address_taken_locals` set forces address-taken
  primitives into Cranelift `StackSlot`s while LLVM (every
  local already has an `alloca`) just returns
  `cx.allocas[target]`. **No borrow checking** — the static
  `Ty::Ref(_)` vs `Ty::Ptr(_)` distinction is what later
  sub-bundles consult to know which pointers are tracked.
  Scope restriction: `&local` only (not `&x.field`,
  `&literal`, `&call()`, `&mut`, or `&` in fn signatures).
  Corpus: `220_borrow_int.gw` → 5, `221_borrow_then_let.gw`
  → 42, `222_borrow_arith.gw` → 30.

- **B.1 — `&mut` + writes is closed** (commit `080bd1a`):
  `KwMut` reserved (lexer + AST + syntax-kind plumbing);
  `parse_type` and `parse_atom` eat an optional `KwMut` after
  a leading `Amp`. `Ty::Ref(RefInner)` migrates to the struct
  variant `Ty::Ref { mutable: bool, inner: RefInner }`
  (`Ty: Copy` preserved — `RefInner` is closed and `bool` is
  `Copy`). `synth_assign` gains a `*r = v` arm gated on
  `mutable: true` (rejects `&T` writes with a dedicated
  diagnostic, rejects non-ref operands with the existing
  `expected &mut T` message). MIR grows `MirStmt::StoreThroughRef
  { ptr, value, ty }`; both backends emit a single
  pointer-typed store at the operand's width (Cranelift
  `fb.ins().store(MemFlags::new(), val, ptr_val, 0)`; LLVM
  `builder.build_store(ptr_val.into_pointer_value(), val)`).
  Still no borrow checking. Corpus: `242_mut_borrow_write.gw`
  → 10 (write through `&mut x`, read back via the original
  name), `243_mut_borrow_then_read.gw` → 42 (write, then
...
- **B.3 — move-tracking dataflow framework is closed**
  (commit `ba39a73`): `gw_borrow` becomes real. Per-fn
  forward dataflow on MIR with lattice = `FxHashSet<Local>`
  of must-be-init locals, join = set intersection, transfer
  adds `Assign / AssignField / Call.dst`. Reads of a local
  not in the must-init set at the read site emit E0400
  `USE_OF_UNINIT_LOCAL` (deduped per local). Phase-1 surface
  is **Copy-by-default** (per fixture 218's pass-by-value
  semantics — classes and slices flow as copies, not Rust
  moves), so the framework catches no actual move-out
  transitions today. What it *does* catch is genuine uninit
  reads from `let x: T;` (no initialiser) and conditional-
  init shapes where only some arms of an `if` write the
  local. Move tracking proper (Move-typed surface + move-out
  invalidations) rides B.4+ — this increment installs the
  dataflow skeleton, worklist iteration, and diagnostic
  plumbing that those increments extend.
  Driver wires the checker between MIR build and codegen;
  any non-empty diag list aborts before codegen so we never
  lower a program that may read garbage.
  MIR side-fix: `lower_match`'s trailing dangling-block
  terminator switched from `Goto(join_bb)` to
  `Terminator::Unreachable` (the path was always dynamically
  unreachable for exhaustive matches but poisoned the
  must-init intersection at `join_bb`). Strict improvement
  for non-exhaustive matches: traps cleanly instead of
  reading garbage from the never-written `result_local`.
  AssignField treated as an init *write* of the dst (not a
  read-requiring-init) because `lower_struct_lit` builds
  aggregates field-by-field on freshly-allocated dsts with
  no prior `Assign`. Partial-init tracking (writing only
  some fields before a whole-aggregate read) rides a future
  sub-bundle alongside field-level move tracking.
  Bootstraps `tests/corpus/fail/borrow/` with three
  fixtures: b01 (use of `let x: i32; return x;`), b02
  (conditional init — only then-arm writes), b03
  (loop-no-init — `while i < 0` body may execute zero
  times). Pass-corpus adds fixture 248 pinning the positive
  both-arms-init case.
  Yield: 1 — `lower_match`'s `Goto(join_bb)` dangling-block
  terminator was a pre-existing latent codegen-correctness
  issue (non-exhaustive matches would have read garbage),
  surfaced by B.3's dataflow flagging the intersection.

- **B.4 — region-origin analysis is closed** (commit
  `fe733b5`): `gw_borrow` grows its second pass alongside
  B.3's init dataflow. Per-fn origin map seeded by scanning
  every `Rvalue::Ref` — parameters anchor as
  `Origin::Param(local)` (caller-owned storage, outlives the
  fn return); let-binding locals anchor as
  `Origin::LocalScope(local)` (callee-frame storage, dies at
  the fn return). Fixpoint-propagated through ref-typed
  `Assign { dst, Use(Local(src)) }` chains and through
  `Terminator::Call { args, dst, .. }` where `dst` is
  ref-typed (union of every ref-typed argument's origins —
  Phase-1 elision rule: no lifetime annotations in
  signatures, so the conservative rule is "the call-result
  reference's origin is any of its reference arguments'
  origins"). On `Return(Operand::Local(r))` where `r` is
  ref-typed, any `LocalScope` anchor in `origins[r]` emits
  `BORROW_OUTLIVES_FN` (E0401) at the borrowed local's
  `LocalDecl.span`. Deduped to one diag per fn so multiple
  return blocks tracing the same dangling chain fire once.
  Zero new lex / parse / typeck / MIR machinery — pure
  analysis on existing MIR shapes (`Rvalue::Ref` was already
  in MIR since B.0; `Ty::Ref { .. }` discriminator was
  already in `gw_typeck` since B.1). Origin sets double as
  the seed data structure B.5's loan-tracking dataflow
  ("which loans are still in scope at point P") will
  consume. Catches the canonical dangling-borrow shapes:
  b04 (`return &x;` direct) and b05 (`let r = &x; return
  r;` indirect — exercises the fixpoint propagation
  through the `Assign { dst: r, Use(Local(tmp)) }` chain).
  Scope restriction: function-local regions only — caller-
  side liveness (`fn longest(a: &i32, b: &i32) -> &i32`
  picker shapes) is correctly accepted because both
  parameters anchor as `Param`, but cross-fn region
  constraints (Polonius's "loans flow across call edges")
  ride B.5. Architecture spec: D.5 step 1.
  Yield: 0 — the lattice's monotone-only-grows shape made
  the convergence argument structural; the
  parameter / let-scope discriminator went through the same
...
- **B.6 — drop elaboration** (pending). Insert `Drop`
  ...
  definitely-uninitialized. Architecture spec: D.5 step 4.

- **B.7 — `defer` / `errdefer` keywords** (pending).
  Lex + parse keywords; MIR inserts the deferred body at
  every scope-exit edge. Architecture spec: D.5 step 5.
  Self-contained at the language-surface level — can
  interleave with the checker ladder.

- **B.8 — safety tiers** (pending). `unsafe { … }` /
  `manual { … }` blocks; `manual` suppresses the aliasing
  check, `unsafe` skips both. Architecture spec: D.5 step
  6. Last sub-bundle in this ladder; allocators / `if let`
  / `orelse` sugar are independent Phase 3 deliverables
  that can interleave.

Architecture Phase-3 entrance criterion is "Phase 2
complete"; we're opening Phase 3 in parallel because
CT.3c+ are corpus-motivated and no corpus program demands
them today. The borrow-check work is contracted by the
architecture regardless of CT.3's state, so the parallel
schedule isn't speculation. Allocators, `?T` sugar, and
`if let` / `orelse` (per architecture Phase-3 deliverables
list) remain unscheduled — they don't block the borrow
checker and can be picked up as the corpus motivates them.
...
#         248_uninit_both_arms_init that pins the both-arms-
#         init shape the dataflow's intersection accepts, on
#         top of B.2's four 244_..247_ call-boundary fixtures
#         and B.1's two 242_..243_ mut-borrow fixtures and
