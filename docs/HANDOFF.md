...
> **Last updated:** 2026-05-14, after closing CT.3a — comptime
> float arithmetic + comparisons (commit `0b3ccba`) on top of
> the block-like-tail widening (`9ac51a1`), implicit-tail-return
> (`579c4f0`), and CT.2's closure. `comptime { 3.14 + 0.5 }`,
> float ordering, float equality (including IEEE-754
> `NaN == NaN` is `false`), and the `Const::Float` materialisation
> arm in MIR all evaluate bit-exactly on both backends.
> **Repo root:** `/Users/silmaril/Documents/GitHub/gw`
> **Workspace tests:** 285 unit + integration, all green.
> **Corpus:** 62 Phase-0 lex+parse snapshots + 248 Phase-1 + 29
...
the 248-program Phase-1 corpus + 4 multi-file projects + the 29
...
with f32 / f64 width narrowing. The remaining CT.3 surface is
strings, classes, optionals, and error unions as the corpus
...
### What 277 corpus programs cover
...
- Phase 2 increment CT.1 + CT.2a + CT.2b + CT.2c + CT.2d +
  ...
  `#[test]` fns). Twenty-nine tracer programs today: `ct1_tracer.gw` → exit
  ...
  which still raises). The accepted shapes inside the block
  are zero-or-more `let pat = init;` statements with
  `IdentPat` patterns, followed by a tail expression of shape
  `IntLit / FloatLit / KwTrue / KwFalse / Path (resolved to
  a let-bound local) / Paren(expr) / Unary(Minus, expr) /
  Block(of-same) / Binary(lhs, op, rhs) / IfExpr` for `op ∈
  {+, -, *, /, %, <, <=, >, >=, ==, !=, &&, ||}` (arithmetic
  / ordering / equality accept both `(Int, Int)` and
  `(Float, Float)` pairs since CT.3a; equality also accepts
  `(Bool, Bool)` from CT.2b; `&& / ||` short-circuit lazily;
...
bit-exactly the same value across all 277 single-file corpus
programs (248 phase1 + 29 phase2_comptime) + 4 multi-file
projects.
...
CT.2d + CT.2e + CT.3a — **CT.2 closed, CT.3 underway**) brings
compile-time evaluation of integer-, bool-, and float-typed
blocks with let-bindings, full control flow, short-circuit
logical operators, and IEEE-754 float arithmetic / comparisons. The
...
- comptime expression-level — **CT.1 + CT.2a + CT.2b + CT.2c
  ...
  Opens the CT.3 family — wider shapes (strings, classes,
  optionals, error unions) ride later CT.3 sub-bundles.

Estimated cost remaining: dozens of hours, distributed between
the remaining CT.3 widenings (strings, classes, optionals, error
unions) and whatever path the `comptime fn` decl-level question
takes. Bug yield so far is **3 caught + 1 deferred-and-now-
resolved** across all twenty closed Phase-2 sub-bundles
(C.1+C.2+M.1+M.2+M.3+O.2+F.1+F.2+F.3+CT.1+CT.2a+CT.2b+CT.2c+CT.2d+CT.2e+impl-tail-ret+block-like-tail+CT.3a
= 0 caught, O.1 = 1 caught, O.3 = 2 caught, CT.1's E0315
"deferred" entry resolved by implicit-tail-return, against
a 12/A.x prediction of ~14-19 — the recombination
...
Phase 1 increment 12. The remaining CT.3 widenings introduce
non-primitive value shapes that the locals env's
`Vec<Option<CtValue>>` doesn't yet store naturally, so the
prediction stays armed — strings, classes, optionals, and
error unions will still be the largest bug surface in Phase 2.
...
#### Remaining Phase-2 sub-bundles: CT.3b+, possibly `comptime fn`
...
- **CT.3b+ comptime over richer types** — strings, classes,
  optionals, error unions as the corpus motivates. The
  `CtValue` enum gains the remaining shapes. Aggregate shapes
  (classes, slices) introduce storage and field-access
  concerns to the locals env — the current dense
  `Vec<Option<CtValue>>` indexing-by-`BindingId.0` only
  stores small Copy variants today. Strings need a borrow
  strategy (likely interned via the source map or via a
  Phase-2 string pool). Optionals / error unions are
  parallel to the runtime `{tag, payload}` layout but the
  payload would be another `CtValue`, suggesting a `Box`-y
  or arena-allocated extension. Bug yield estimate: ~1-2
  per shape introduction (strings: borrow-lifetime
  interaction with `EvalCx`'s source-map borrow; aggregates:
  locals-env storage shape; optionals: payload-vs-tag
  materialisation). Suggested ordering: motivated by corpus
  need rather than speculative widening — pick the shape
  the next non-trivial comptime use case actually asks for.

Suggested ordering: CT.3b+ when a corpus program motivates a
...
# expect tip: HANDOFF refresh after CT.3a (this commit),
#             0b3ccba (CT.3a comptime float arithmetic +
#             comparisons), 2d069a6 (HANDOFF refresh after
#             block-like-tail), 9ac51a1 (block-like-tail parser
#             widening + divergent-tail discard), c215ce4
#             (HANDOFF refresh after impl-tail-ret), 579c4f0
#             (implicit-tail-return for bare-expression tails),
#             b8d8cc5 (HANDOFF refresh after CT.2e), 9d062d3
#             (CT.2e comptime short-circuit && / ||), 2611606
#             (HANDOFF refresh after CT.2d), a03c361 (CT.2d
#             comptime if/else) at the bottom of head -10.
...
# expect: 29 (ct1_tracer + 4 ct2a_* + 4 ct2b_* + 4 ct2c_* + 5
...
`lower_literal`'s runtime `FloatLit` path. The 248-program
phase1 corpus + 4 multi-file projects + 29 phase2_comptime
tracers (`ct1_tracer.gw` → 4; `ct2a_*` 4 tracers; `ct2b_*` 4
...
`206_tail_while_u0.gw` → 0; and 5 CT.3a fixtures
...
`else` arm) are the direct test surface for every one of
those arrows, exercised through both backends in CI.
