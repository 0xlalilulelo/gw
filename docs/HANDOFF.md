...
> **Last updated:** after Phase-2 increments O.2 (`?T` match
> patterns + `nil` arm) and O.3 (`!T` error union + postfix
> `!`-assert). The optional / error-union sub-bundle is now closed.
> **Repo root:** `/Users/silmaril/Documents/GitHub/gw`
> **Workspace test count:** 196 unit + integration tests, all green.
> **Corpus size:** 61 Phase-0 lex+parse snapshots + 241 Phase-1 / Phase-2 run-tests.
...
> match sub-bundle (M.1 + M.2 + M.3), and optional / error-union
> sub-bundle (O.1 + O.2 + O.3) all closed. `?T` and `!T` lower as
> 2-field `{tag, payload}` aggregates with `T → ?T` and `T → !T`
> coercions at let-init / return / call-arg via a single canonical
> wrap helper. `??` reads the optional's tag and lazily evaluates a
> default; `expr!` reads the error-union's tag and traps on err.
> `match opt { nil => ..., _ => ... }` exercises the existing
> decision-tree pattern infrastructure. Three bugs caught across
> the entire Phase-2 entry (O.1: 1, O.3: 2; everything else 0) —
> all three were latent multi-site predicate-canonicalisation
> bugs that the dual-backend test surfaced when value-level novelty
> entered. Cleanup #1 (default `-> u0` on missing return type)
...
M.2 + M.3), and the optional / error-union sub-bundle (O.1 + O.2
+ O.3) end-to-end. `c"..."` literals lex / parse / typeck / MIR
/ Cranelift / LLVM; `[*:0]u8` is a parsed-and-type-system-
distinct sentinel pointer that decays to `*u8` at extern call
sites and at `let` annotations. `match` accepts integer-literal,
bool-literal, inclusive-range (`lo..=hi`), or-pattern
(`a | b | c`), wildcard, and (for `?T` scrutinees) `nil`
patterns; arms unify against a single result type. `?T` and
`!T` land as 2-field `{tag: u8, payload: T}` aggregates with
the same layout (Phase-2 minimum: integer + bool inners); `nil`
adopts `?T`, bare `T` coerces to `?T` and to `!T` at let-init /
return / call-arg through a single canonical wrap helper. `??`
reads the optional's tag and lazily evaluates a default;
`expr!` reads the error-union's tag and traps on err. The
MIR-side decision-tree helper `lower_pattern_test` recurses for
or-patterns, emits short-circuit pairs for ranges, and reads
the tag byte for `nil` patterns; `??` and `expr!` emit
3-block tag-test CFGs. Fifteen new corpus programs (eight
match + three c-string + four `?T`/`!T`) exercise the full
...
is comptime + frequencies/modules — the architectural heavy lift,
...
### Active crate roles (≈7 700 LoC of compiler logic)
...
its three pre-comptime sub-bundles: c-strings (C.1 + C.2 + cleanup
#1), match (M.1 + M.2 + M.3), and optional / error-union (O.1 +
O.2 + O.3). The remaining Phase-2 work is comptime + frequencies
/ modules — the architectural heavy lift, deserves its own session.
...
Estimated cost remaining: dozens of hours, mostly in the comptime
+ modules sub-bundle. Bug yield so far is 3 across all eight
closed Phase-2 sub-bundles (C.1+C.2+M.1+M.2+M.3+O.2 = 0, O.1 = 1,
O.3 = 2, against a 12/A.x prediction of ~6 — the recombination
sub-bundles (M.x and O.2) under-shot prediction because they
reused already-validated value-level shapes; the value-level-
novel sub-bundles (O.1 and O.3) hit prediction exactly). The
comptime sub-bundle introduces a brand-new evaluator state +
multi-file build orchestration, so the prediction stays armed —
this will be the largest bug surface in Phase 2 by an order of
magnitude.
...
#### Only remaining Phase-2 sub-bundle: comptime + modules

After O.3 closes the optional / error-union sub-bundle, the only
remaining Phase-2 work is comptime + frequencies (modules). This
is the architectural heavy lift and deserves its own session.

- **comptime + frequencies (modules)** — new `arsenal_comptime`
  crate (currently a stub), multi-file builds, module resolution.
  Gates `cipher` (the package manager) and the manifest-driven
  driver UX. Bug yield estimate: highest of any Phase-2
  sub-bundle by an order of magnitude — the comptime evaluator
  is genuinely new code (no parallel implementation to crib
  from), multi-file build orchestration touches the driver +
  resolve + typeck end-to-end, and module-level `use` /
  `frequencies` declarations require new AST + parser surface
  area. The 12/A.x heuristic predicts ~3-5 bugs across the
  bundle; without value-level-novel surface, even the lowest
  estimate is several genuinely-new shapes.

Suggested sub-bundling (rough — finalise at session start):
- **F.1 multi-file tracer** — driver reads multiple `.gw` files
  into one SourceMap, lex / parse each separately, resolve
  cross-file (no namespace yet), one MirProgram with all fns
  visible. Tracer: 2-file project where `main.gw` calls a fn
  defined in `lib.gw`. No `use` / `frequencies` yet. Smallest
  meaningful step into multi-file.
- **F.2 `frequencies` / `use` declarations** — actual module
  syntax. `frequencies foo;` declares a module; `use foo;` (or
  `use foo::bar;`) imports.
- **CT.1 comptime tracer** — `comptime fn foo() -> i32 { return
  42; }` evaluates at compile time; runtime calls become the
  constant. Smallest comptime step: pure-int constant evaluator.
- **CT.2 comptime over more types** — extend the evaluator to
  handle arithmetic, bool, classes, strings…
- **CT.3 comptime + frequencies integration** — `comptime use
  foo;` etc. (Tentative; depends on what F.x and CT.x lock in.)

The session-start move: read this section, choose F.1 vs CT.1 as
the very first step (both viable; F.1 has the simpler
infrastructure but doesn't prove the comptime evaluator works;
CT.1 proves the evaluator but doesn't help multi-file builds).
Either way, surface design decisions before writing code — this
is the largest sub-bundle entry of Phase 2.
...
# expect tip: HANDOFF refresh after O.2/O.3 (this commit), 5282bc8
#             (O.3 !T error union), c555777 (O.2 ?T match patterns),
#             8a270dc (HANDOFF after O.1), 7c46d5b (O.1 ?T tracer),
#             598cece (HANDOFF after match sub-bundle), 2d85e65 (M.3
#             range + or-patterns), 7d9c04d (M.2 bool + stmt-position),
#             183e5b8 (M.1 match tracer), d8a217e (HANDOFF after
#             C.1/C.2 + cleanup #1) at the bottom of head -10.
...
241-program corpus is the direct test surface for every one of
